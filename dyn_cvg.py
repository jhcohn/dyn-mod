...
def do_emcee(nwalkers=250, burn=100, steps=1000, printer=0, all_free=True, parfile=None, pool=None, save=True, nthr=4):
    ...
    f = open("chain.pkl", "wb")
    f.close()
    for ii, result in enumerate(sampler.sample(p0, iterations=steps, store=True)):  # iterations=500, storechain=False
        ii += 1
    ...
    if save:
        ...
        with open(out_flatchain, 'wb') as newfile:  # 'wb' because binary format
            pickle.dump(np.ndarray.flatten(sampler.chain), newfile, pickle.HIGHEST_PROTOCOL)
    ...
    if printer:
        ...
        fig = corner.corner(samples, labels=[v for v in free_p.keys()], truths=[v for v in free_p.values()])
...
if __name__ == "__main__":
    ...
    flatchain = do_emcee(nwalkers=int(args['w']), burn=int(args['b']), steps=int(args['s']), printer=1,
                         parfile=args['parfile'], all_free=True, pool=None, save=True, nthr=8)  # 250 100 100
