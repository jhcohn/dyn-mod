...
class ModelGrid:
    ...
    def create_grid(self):
        ...
        if len(x_obs_ac) % 2. == 0:  # if even
            ...
            for i in range(len(x_obs_ac)):
                x_obs_ac[i] = self.resolution * (i - x_ctr) / self.os  # (arcsec/pix) * N_subpixels / (subpixels/pix) = arcsec
        else:  # elif odd
        ...
        x_bh_ac = (x_loc - x_ctr / self.os) * self.resolution  # units (pix - subpix/[subpix/pix]) * (arcsec/pix) = arcsec
        ...
        y_disk_ac = (y_obs_ac[:, None] - y_bh_ac) * np.cos(self.theta) - (x_obs_ac[None, :] - x_bh_ac) * np.sin(self.theta)  # arcsec
        x_disk = (x_obs[None, :] - x_bhoff) * np.cos(self.theta) + (y_obs[:, None] - y_bhoff) * np.sin(self.theta)  # 2d array
        y_disk = (y_obs[:, None] - y_bhoff) * np.cos(self.theta) - (x_obs[None, :] - x_bhoff) * np.sin(self.theta)  # 2d array
        ...
        if self.menc_type == 0:  # if calculating v(R) due to stars directly from MGE parameters
            ...
            v_c = mvm.mge_vcirc(surf_pots * self.ml_ratio, sigma_pots, qobs, np.rad2deg(self.inc), 0., self.dist, test_rad)  # v_c,star
        ...
        v_los = sign * abs(vel * np.cos(alpha) * np.sin(self.inc))  # THIS IS CURRENTLY CORRECT

        # INCLUDE NEW RADIAL VELOCITY TERM
        # v_los = v_los*cos(phi)*sin(inc)  - vrad*sin(phi)*sin(inc) (+ or -)
        vrad_sign = y_disk / abs(y_disk)
        v_los += self.vrad * vrad_sign * abs(np.sin(alpha) * np.sin(self.inc))  # See notebook for derivation!
        # With this sign convention: vrad > 0 -> outflow; vrad < 0 -> inflow!
    ...
    def convolve_cube(self):
        ...
        # BUCKET need to do something about "rebin"
        ...
        for z in range(len(self.z_ax)):
            self.convolved_cube[z, :, :] = convolution.convolve(intrinsic_cube[z, :, :], self.beam)  # CONVOLVE CUBE WITH BEAM
    ...
    def chi2(self):
        ...
        print(len(masked_pix), len(self.z_ax))
        n_pts = len(masked_pix) * len(self.z_ax)
        print(n_pts)
        ...
        #cf = rebin(rebin(subpix_deconvolved, ds), ds)[0]
        ...
        for z in range(self.zrange[0], self.zrange[1]):  # for each relevant freq slice (ignore slices with only noise)
            chi_sq += np.sum((ap_4[z_ind] - data_4[z_ind])**2 / self.noise[z_ind]**2)  # calculate chisq!
            cs.append(np.sum((ap_4[z_ind] - data_4[z_ind])**2 / self.noise[z_ind]**2))  # chisq per slice
