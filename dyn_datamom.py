...
class ModelGrid:
    ...
    def vor_moms(self, incl_beam, snr=10):
        ...
        # CALCULATE NUMERATOR AND DENOMINATOR USED IN MOMENT 1 & 2, JUST FOR DATA
        data_numerator = np.zeros(shape=(len(clipped_mask[0]), len(clipped_mask[0][0])))
        data_denominator = np.zeros(shape=(len(clipped_mask[0]), len(clipped_mask[0][0])))
        for zi in range(len(clipped_mask)):
        ...
        # CALCULATE MOMENT 2
        d2_num = np.zeros(shape=(len(clipped_mask[0]), len(clipped_mask[0][0])))
        d2_n2 = np.zeros(shape=(len(clipped_mask[0]), len(clipped_mask[0][0])))
        d2_den = np.zeros(shape=(len(clipped_mask[0]), len(clipped_mask[0][0])))
        for zi in range(len(clipped_mask)):
