...
class ModelGrid:
    ...
    def grids(self):
        ...
        # PLOT FLUX MAP/LUCY MAP AND RADIUS MAP. FIRST FLUX/LUCY (WITH OR WITHOUT RECTANGLE SUBCUBE, BEAM)
        '''  #
        full_xac = np.zeros(shape=len(self.input_data[0]))
        full_yac = np.zeros(shape=len(self.input_data[0][0]))
        for i in range(len(full_xac)):
            full_xac[i] = self.resolution * (i - len(full_xac) / 2)
        for i in range(len(full_yac)):
            full_yac[i] = self.resolution * (i - len(full_yac) / 2)
        midz = int(self.zrange[0] + (self.zrange[1] - self.zrange[0]) / 2)
        ...
        for zi in range(len(input_data)):
            collapsed_fluxes_vel += self.input_data[zi] * data_mask[zi] * v_width
        #ax.imshow(collapsed_fluxes_vel, origin='lower', extent=extent)  # PLOT FLUX
        ax.imshow(self.lucy_out, origin='lower', extent=extent)  # OR PLOT LUCY
        # Create a Rectangle patch
        rect = patches.Rectangle((full_xac[self.xyrange[0]], full_yac[self.xyrange[2]]),
                                 full_xac[self.xyrange[1]]-full_xac[self.xyrange[0]],
                                 full_yac[self.xyrange[3]] - full_yac[self.xyrange[2]], linewidth=2, edgecolor='w',
                                 facecolor='none')
        # ax.add_patch(rect)  # Add the patch to the Axes
        # x_fwhm=0.197045, y_fwhm=0.103544  # in arcsec
        print(params['PAbeam'])
        ell = patches.Ellipse((2., -2.), 0.197045, 0.103544, angle=90.+params['PAbeam'],
                             linewidth=2, edgecolor='w', fill=False)  # np.rad2deg(params['theta_ell']))
        ax.add_patch(ell)  # Add the ellipse patch to the axes
        ...
        cbar.set_label(r'R [pc]')
        ax.set_xlabel(r'x [arcsec]')
        ax.set_ylabel(r'y [arcsec]')
        ax.arrow(0, 0, 0, 0.35, color='w', head_width=0.02)
        ax.arrow(0, 0, 0.35, 0, color='w', head_width=0.02)
        ax.arrow(0, 0, 0.35*np.cos(self.theta), 0.35*np.sin(self.theta), color='w', ls='--', head_width=0.02)
        ax.arrow(0, 0, -0.35*np.sin(self.theta), 0.35*np.cos(self.theta), color='w', ls='--', head_width=0.02)
        ax.plot(0, 0, 'w*')
        plt.show()
        print(oop)
        # '''
        ...
        extent = [x_obs_ac[0], x_obs_ac[-1], y_obs_ac[0], y_obs_ac[-1]]
        im = ax.imshow(v_los, origin='lower', extent=extent, cmap='RdBu_r', vmax=np.max([v_los, -v_los]),
                       vmin=np.amin([v_los, -v_los]))
        from mpl_toolkits.axes_grid1 import make_axes_locatable
        divider = make_axes_locatable(ax)  # set colorbar same size as plot
        cax = divider.append_axes("right", size="5%", pad=0.05)  # set colorbar same size as plot
        cbar = plt.colorbar(im, cax=cax)
        cbar.set_label(r'v$_{\rm{los}}$ [km/s]')
        ax.set_xlabel(r'x [arcsec]')
        ax.set_ylabel(r'y [arcsec]')
        plt.show()
        print(oop)
