...
def test_dyn(theta, par_dict=None, mod_ins=None, params=None):
    ...
    chi2 = dm.model_grid(
        # FREE PARAMETERS
        x_loc=params['xloc'],  # par_dict.keys().index('xloc') (and etc beneath)
        # x_loc=theta[par_dict.index('xloc')],
...
if __name__ == "__main__":
    ...
    print('here we go')
    flatchain = do_emcee(nwalkers=int(args['w']), burn=int(args['b']), steps=int(args['s']), printer=0, parfile=args['parfile'], all_free=True, pool=pool, save=True, nthr=12, q=False)
