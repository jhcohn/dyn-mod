    ...
    with open(out_tab, 'r') as efile:  # '/Users/jonathancohn/Documents/mge/out_tab1_saved.txt'
        for line in efile:
            if not line.startswith('#'):
                cols = line.split()
                intensity_characters = ''
                intensity_unc_characters = ''
                parens = 0
                for char in cols[1]:  # for each character in second column, e.g. "308.72("
                    if char == '(' or char == ')':  # skip parentheses (count them)
                        parens += 1
                    elif parens == 0:
                        intensity_characters += char  # if haven't reached parentheses yet, we're on intensity
                    elif parens == 1:
                        intensity_unc_characters += char
                if len(cols) > 2 and intensity_unc_characters == '':
                    for char in cols[2]:
                        if char == ')':  # skip parentheses still (still count them)
                            parens += 1
                        elif parens == 1:  # starting at 1 because there's 1 at the end of cols[1]
                            intensity_unc_characters += char
                if len(cols) == 2:  # the central intensity (at sma=0) has no associated uncertainty in the table
                    intensity_unc_characters = '0.0'
                smas.append(float(cols[0]))
                intens.append(float(intensity_characters))
                i_unc.append(float(intensity_unc_characters))
    smas.reverse()
    intens.reverse()
    # print(smas)
    # print(intens)
    return smas, intens, i_unc
...
def sb_compare3(in_filename, out_filename, just_out, table_img, table_mod, table_ben, qbounds=[0., 1.], sky=None,
    ...
    smas_img, intens_img, img_unc = tab_to_sb(table_img)
    smas_mod, intens_mod, mod_unc = tab_to_sb(table_mod)
    smas_ben, intens_ben, ben_unc = tab_to_sb(table_ben)
...
def sb_prof_compare(in_filename, out_filename, just_out, table_img, table_mod, qbounds=[0., 1.], sky=None, scale=0.1,
    ...
    if es:
        ax[0].set_ylabel('Mean counts (electrons)\nalong semi-major axis')
    #else:
    #    ax[0].set_ylim(8e-2, 3.*10**3)  # 2e-3, 3.*10**3
    ax[0].set_xlim(0.03, 150.)  # (0.5, 400.)
    ax[0].set_ylim(ylo, yhi)  # 2e-3, 3.*10**3 ;; 200., 3. * 10 ** 6
    ax[1].set_ylim(-ryl, ryl)  # -0.4, 0.4)  # yl = 0.15

    f = interpolate.interp1d(smas_mod, intens_mod, fill_value="extrapolate")  # x, y
    ferr = interpolate.interp1d(smas_mod, mod_unc, fill_value="extrapolate")  # x, y
    mod_unc_samex = ferr(smas_img)  # not dependent on sky
    if sky is not None and not hasattr(sky, "__len__"):
        # IF SKY IS FIT IN GALFIT, IT IS INCLUDED IN THE OUTPUT MODEL.FITS! SO, SUBTRACT FROM MODEL *ONLY* IF SKY IS
        # ALSO ALREADY SUBTRACTED FROM THE IMAGE! OTHERWISE, DON'T SUBTRACT FROM THE MODEL!!
        intens_mod_samex = f(smas_img) - sky
    else:
        intens_mod_samex = f(smas_img)

    # intens_mod_samex = [ims * 898.467164 / 1354.46 for ims in intens_mod_samex]
    # residual error: resid = r = (x-y)/x = z/x --> err = r*sqrt((sigz/z)**2 + (sigx/x)**2)
    # sigz = r*sqrt((sigx/x)**2+(sigy/y)**2) --> r*sqrt((r/z)**2 * (sigx/x)**2+(sigy/y)**2 + (sigx/x)**2)
    # err = ((x-y)/x) * sqrt(((x-y)/x**2 + 1) * (sigx/x)**2 + (sigy/y)**2)
    # err = ((intens_img-intens_mod_samex)/intens_img) * sqrt(((intens_img-intens_mod_samex)/intens_img**2 + 1) * (img_unc/intens_mod)**2 + (mod_unc_samex/intens_mod_samex)**2)

    print(len(img_unc), len(intens_img), len(mod_unc_samex), len(intens_mod_samex))

    intens_img = np.array(intens_img)
    img_unc = list(reversed(img_unc))
    img_err = np.zeros(shape=(2,len(img_unc)))
    mod_unc_samex = list(reversed(mod_unc_samex))
    mod_err = np.zeros(shape=(2,len(mod_unc_samex)))
    resid_err = np.zeros(shape=(2,len(mod_unc_samex)))
    fullresid_err = np.zeros(shape=(2,len(mod_unc_samex)))
    corrresid_err = np.zeros(shape=(2,len(mod_unc_samex)))
    resid_err_cor = np.zeros(shape=(2,len(mod_unc_samex)))
    for iu in range(len(img_unc)):
        img_err[0,iu] = img_unc[iu]
        img_err[1,iu] = img_unc[iu]
        mod_err[0,iu] = mod_unc_samex[iu]
        mod_err[1,iu] = mod_unc_samex[iu]
        resid_err[0,iu] = np.sqrt(mod_unc_samex[iu]**2 + img_unc[iu]**2) / intens_img[iu]
        resid_err[1,iu] = np.sqrt(mod_unc_samex[iu]**2 + img_unc[iu]**2) / intens_img[iu]
        corrresid_err[0, iu] = ((intens_img[iu] - intens_mod_samex[iu]) / intens_img[iu]) *\
                               np.sqrt((img_unc[iu]**2 + mod_unc_samex[iu]**2)
                                       / (intens_img[iu] - intens_mod_samex[iu])**2 + (img_unc[iu] / intens_img[iu])**2)
        corrresid_err[1, iu] = ((intens_img[iu] - intens_mod_samex[iu]) / intens_img[iu]) *\
                               np.sqrt((img_unc[iu]**2 + mod_unc_samex[iu]**2)
                                       / (intens_img[iu] - intens_mod_samex[iu])**2 + (img_unc[iu] / intens_img[iu])**2)
        resid_err_cor[0, iu] = (1 - intens_mod_samex[iu] / intens_img[iu]) *\
                               np.sqrt((img_unc[iu] / intens_img[iu])**2 +
                                       (mod_unc_samex[iu] / intens_mod_samex[iu])**2)
        resid_err_cor[1, iu] = (1 - intens_mod_samex[iu] / intens_img[iu]) *\
                               np.sqrt((img_unc[iu] / intens_img[iu])**2 +
                                       (mod_unc_samex[iu] / intens_mod_samex[iu])**2)
        fullresid_err[0,iu] = ((intens_img[iu]-intens_mod_samex[iu])/intens_img[iu]) *\
                              np.sqrt(((intens_img[iu]-intens_mod_samex[iu])/intens_img[iu]**2 + 1) *
                                      (img_unc[iu]/intens_mod_samex[iu])**2 +
                                      (mod_unc_samex[iu]/intens_mod_samex[iu])**2)
        fullresid_err[1,iu] = ((intens_img[iu]-intens_mod_samex[iu])/intens_img[iu]) *\
                              np.sqrt(((intens_img[iu]-intens_mod_samex[iu])/intens_img[iu]**2 + 1) *
                                      (img_unc[iu]/intens_mod_samex[iu])**2 +
                                      (mod_unc_samex[iu]/intens_mod_samex[iu])**2)
    #print(smas_img[-1] / scale, img_err[:,0], img_err[:,-1], 'hi')  # uh-oh, they're reversed?! FIXED by reversing list
    #print(smas_img[1] / scale, img_err[:,1], img_err[:,-2], 'hi')  # uh-oh, they're reversed?! FIXED by reversing list
    # print(smas_img[20] / scale, img_err[:, 20], 'hi')  # GOOD NOW AFTER REVERSING LISTS!

    #ax[0].loglog(smas_img, np.array(intens_img), 'b+', label=r'Data (profile from Ellipse task)', linewidth=2)  # 'b--'
    #ax[0].loglog(smas_img, intens_mod_samex, 'r*', label=r'GALFIT Model (profile from Ellipse task)', linewidth=2)  # 'r-'
    ei = ax[0].errorbar(smas_img, intens_img, yerr=img_err, fmt='bo',
                   label=r'Data (profile from Ellipse task)', linewidth=2)  # 'b--'
    ei[-1][0].set_linestyle('-')  # ei[-1][0] is the LineCollection objects of the errorbar lines
    em = ax[0].errorbar(smas_img, intens_mod_samex, yerr=mod_err, fmt='r*',
                   label=r'GALFIT Model (profile from Ellipse task)', linewidth=2)  # 'r-'
    em[-1][0].set_linestyle('--')  # ei[-1][0] is the LineCollection objects of the errorbar lines
    ax[0].set_xscale('log')
    ax[0].set_yscale('log')
    # , with t_exp~898

    # ax[1].plot(smas_img, (np.array(intens_img) - np.array(intens_mod_samex)) / (np.array(intens_img)), 'ko', markersize=4)
    ax[1].errorbar(smas_img, (np.array(intens_img) - np.array(intens_mod_samex)) / (np.array(intens_img)),
               yerr=resid_err_cor, fmt='ko', markersize=4)
    ax[1].axhline(y=0., color='k', ls='--')
...
''' #### NGC 384, adjusted, sky, n9, TEST WITH SKY=100 MAKE SURE SKY INCLUDED CORRECTLY
sb_prof_compare(fj+'NGC0384_F160W_drz_sci_adjusted.fits',  # for x0, y0 used: 1458, 1368
                gf+'n384/galfit_out_n384_reg_linear_hadj_sky_n9.fits',
                gf+'n384/galfit_out_n384_reg_linear_hadj_sky_n9_model.fits',
                gf+'n384/out_tab_n384_reg_linear_hadj_sky_n9_sky100test_img.txt',
                gf + 'n384/out_tab_n384_reg_linear_hadj_sky_n9_sky100test_mod.txt', scale=0.06, modtype='reg', es=True,
                sky=100., mask=fj+'NGC0384_F160W_drz_mask.fits', glx='NGC 384', ryl=1., ylo=3, yhi=1.2e6)
print(oop)
#### NGC 384, adjusted, sky, n9, TEST WITH SKY=100, MAKE SURE SKY INCLUDED CORRECTLY '''

''' #### NGC 384, adjusted, sky, n9
#plot3(gf+'n384/galfit_out_n384_reg_linear_hadj_sky_n9.fits', clipreg=[1270,1470,1355,1555], indivs=False,
#      cliprange=True)
sb_prof_compare(fj+'NGC0384_F160W_drz_sci_adjusted.fits',  # for x0, y0 used: 1458, 1368
                gf+'n384/galfit_out_n384_reg_linear_hadj_sky_n9.fits',
                gf+'n384/galfit_out_n384_reg_linear_hadj_sky_n9_model.fits',
                gf+'n384/out_tab_n384_reg_linear_hadj_sky_n9_img.txt',
                gf + 'n384/out_tab_n384_reg_linear_hadj_sky_n9_mod.txt', scale=0.06, modtype='reg', es=True,
                mask=fj+'NGC0384_F160W_drz_mask.fits', glx='NGC 384', ryl=1., ylo=3, yhi=1.2e6)
# sky=-5.5534, DON'T INCLUDE SKY HERE BC IT'S ALREADY INCLUDED IN THE OUTPUT GALFIT MODEL!!!!
# see e.g.: https://github.tamu.edu/joncohn/gas-dynamical-modeling/wiki/Week-of-2020-02-17!!!!
print(oop)
#### NGC 384, adjusted, sky n9 '''
''' #### NGC 384, adjusted, sky, n10
#plot3(gf+'n384/galfit_out_n384_reg_linear_hadj_sky_n10.fits', clipreg=[1270,1470,1355,1555], indivs=False,
#      cliprange=True)
sb_prof_compare(fj+'NGC0384_F160W_drz_sci_adjusted.fits',  # for x0, y0 used: 1458, 1368
                gf+'n384/galfit_out_n384_reg_linear_hadj_sky_n10.fits',
                gf+'n384/galfit_out_n384_reg_linear_hadj_sky_n10_model.fits',
                gf+'n384/out_tab_n384_reg_linear_hadj_sky_n10_img.txt',
                gf + 'n384/out_tab_n384_reg_linear_hadj_sky_n10_mod.txt', scale=0.06, modtype='reg', es=True,
                mask=fj+'NGC0384_F160W_drz_mask.fits', glx='NGC 384', ryl=1., ylo=3, yhi=1.2e6)
# sky=-5.5534, DON'T INCLUDE SKY HERE BC IT'S ALREADY INCLUDED IN THE OUTPUT GALFIT MODEL!!!!
# see e.g.: https://github.tamu.edu/joncohn/gas-dynamical-modeling/wiki/Week-of-2020-02-17!!!!
print(oop)
#### NGC 384, adjusted, sky n10 '''
''' #### NGC 384, adjusted, extended mask to cover neighbor galaxy, n10
#plot3(gf+'n384/galfit_out_n384_reg_linear_hadj_extmask_n10.fits', clipreg=[1270,1470,1355,1555], indivs=False,
#      cliprange=True)
sb_prof_compare(fj+'NGC0384_F160W_drz_sci_adjusted.fits',  # for x0, y0 used: 1458, 1368
                gf+'n384/galfit_out_n384_reg_linear_hadj_extmask_n10.fits',
                gf+'n384/galfit_out_n384_reg_linear_hadj_extmask_n10_model.fits',
                gf+'n384/out_tab_n384_reg_linear_hadj_extmask_n10_img.txt',
                gf + 'n384/out_tab_n384_reg_linear_hadj_extmask_n10_mod.txt', scale=0.06, modtype='reg', es=True,
                sky=0., mask=fj+'NGC0384_F160W_drz_mask_extended.fits', glx='NGC 384', ryl=1., ylo=3, yhi=1.2e6)
print(oop)
#### NGC 384, adjusted, extended mask to cover neighbor galaxy, n10 '''
...
print(oop)
#### NGC 384, adjusted, n10 '''
''' #### NGC 384, adjusted ncombine 7, n10
#plot3(gf+'n384/galfit_out_n384_reg_linear_hadjn7_n10.fits', clipreg=[1270,1470,1355,1555], indivs=False,
#      cliprange=True)
sb_prof_compare(fj+'NGC0384_F160W_drz_sci_adjusted_n7.fits',  # for x0, y0 used: 1457.9565, 1367.7228
                gf+'n384/galfit_out_n384_reg_linear_hadjn7_n10.fits',
                gf+'n384/galfit_out_n384_reg_linear_hadjn7_n10_model.fits',
                gf+'n384/out_tab_n384_reg_linear_hadjn7_n10_img.txt',
                gf + 'n384/out_tab_n384_reg_linear_hadjn7_n10_mod.txt', scale=0.06, modtype='reg', es=True, sky=0.,
                mask=fj+'NGC0384_F160W_drz_mask.fits', glx='NGC 384', ryl=1., ylo=30., yhi=3e5)
print(oop)
#### NGC 384, adjusted ncombine 7, n10 '''

# ''' #### PGC 11179, adjusted, in2, beyond3fixed
#plot3(gf+'p11179/galfit_out_p11179_reg_linear_hadj_in2_beyond3fixed.fits', clipreg=[1300,1500,1370,1570], indivs=False,
#      cliprange=True)
sb_prof_compare(fj+'PGC11179_F160W_drz_sci_adjusted.fits',
                gf+'p11179/galfit_out_p11179_reg_linear_hadj_in2_beyond3fixed.fits',
                gf+'p11179/galfit_out_p11179_reg_linear_hadj_in2_beyond3fixed_model.fits',
                gf+'p11179/out_tab_p11179_reg_linear_hadj_in2_beyond3fixed_img.txt',
                gf + 'p11179/out_tab_p11179_reg_linear_hadj_in2_beyond3fixed_mod.txt', scale=0.06, modtype='reg',
                es=True, mask=None, glx='PGC 11179', ryl=1., ylo=3., yhi=8e5)  # GUESS X0, Y0 = 1466 1398
print(oop)
#### PGC 11179, adjusted, in2, beyond3fixed '''
''' #### PGC 11179, adjusted, n9, inner
#plot3(gf+'p11179/galfit_out_p11179_reg_linear_hadj_n9_inner.fits', clipreg=[1300,1500,1370,1570], indivs=False,
#      cliprange=True)
sb_prof_compare(fj+'PGC11179_F160W_drz_sci_adjusted.fits', gf+'p11179/galfit_out_p11179_reg_linear_hadj_n9_inner.fits',
                gf+'p11179/galfit_out_p11179_reg_linear_hadj_n9_inner_model.fits',
                gf+'p11179/out_tab_p11179_reg_linear_hadj_n9_inner_img.txt',
                gf + 'p11179/out_tab_p11179_reg_linear_hadj_n9_inner_mod.txt', scale=0.06, modtype='reg', es=True,
                mask=None, glx='PGC 11179', ryl=1., ylo=3., yhi=8e5)
# X0, Y0 guess: 1465.8832, 1398.2582
print(oop)
#### PGC 11179, adjusted, n9, inner '''

''' #### PGC 11179, adjusted ncombine 7, n9
plot3(gf+'p11179/galfit_out_p11179_reg_linear_hadjn7_n9.fits', clipreg=[1300,1500,1370,1570], indivs=False,
      cliprange=True)
sb_prof_compare(fj+'PGC11179_F160W_drz_sci_adjusted_n7.fits',  # X0,Y0 guess: 1465.8585 1398.2369
                gf+'p11179/galfit_out_p11179_reg_linear_hadjn7_n9.fits',
                gf+'p11179/galfit_out_p11179_reg_linear_hadjn7_n9_model.fits',
                gf+'p11179/out_tab_p11179_reg_linear_hadjn7_n9_img.txt',
                gf+'p11179/out_tab_p11179_reg_linear_hadjn7_n9_mod.txt', scale=0.06, modtype='reg', es=True, sky=0.,
                mask=None, glx='PGC 11179', ryl=1., ylo=30., yhi=3e5)
print(oop)
#### PGC 11179, adjusted ncombine 7, n9 '''
''' #### PGC 11179, adjusted ncombine 7, n9
plot3(gf+'p11179/galfit_out_p11179_reg_linear_hadjn7_skyallpars_n8.fits', clipreg=[1300,1500,1370,1570], indivs=False,
      cliprange=True)
sb_prof_compare(fj+'PGC11179_F160W_drz_sci_adjusted_n7.fits',  # X0,Y0 guess: 1465.8585 1398.2369
                gf+'p11179/galfit_out_p11179_reg_linear_hadjn7_skyallpars_n8.fits',
                gf+'p11179/galfit_out_p11179_reg_linear_hadjn7_skyallpars_n8_model.fits',
                gf+'p11179/out_tab_p11179_reg_linear_hadjn7_skyallpars_n8_img.txt',
                gf+'p11179/out_tab_p11179_reg_linear_hadjn7_skyallpars_n8_mod.txt', scale=0.06, modtype='reg', es=True,
                sky=0., mask=None, glx='PGC 11179', ryl=1., ylo=30., yhi=3e5)
print(oop)
#### PGC 11179, adjusted ncombine 7, n9 '''
...
sb_prof_compare(fj+'PGC11179_F160W_drz_sci_adjusted.fits',
                ...
                gf+'p11179/galfit_out_p11179_reg_linear_hadj_skyallpars_n9_model.fits',  # _modelwithsky.fits
                gf+'p11179/out_tab_p11179_reg_linear_hadj_skyallpars_n9_img.txt',
                gf + 'p11179/out_tab_p11179_reg_linear_hadj_skyallpars_n9_mod.txt',  # _modwithsky.txt
                scale=0.06, modtype='reg', es=True, mask=fj+'PGC11179_F160W_drz_mask', glx='PGC 11179',
                ryl=1., ylo=3., yhi=8e5)
# X0, Y0 guess (for _model.fits, _mod.txt) -> 1465.88, 1398.26
# sky=skybkgd, DON'T INCLUDE SKY HERE BC IT'S ALREADY INCLUDED IN THE OUTPUT GALFIT MODEL!!!!
# see e.g.: https://github.tamu.edu/joncohn/gas-dynamical-modeling/wiki/Week-of-2020-02-17!!!!
print(oop)  #
#### PGC 11179, adjusted, n9, skyfree allpars '''
...
sb_prof_compare(fj+'PGC11179_F160W_drz_sci_adjusted.fits',
                ...
                gf+'p11179/galfit_out_p11179_reg_linear_hadj_pafree_skyallpars_n8_model.fits',  # _modelwithsky.fits
                gf+'p11179/out_tab_p11179_reg_linear_hadj_pafree_skyallpars_n8_img.txt',
                gf + 'p11179/out_tab_p11179_reg_linear_hadj_pafree_skyallpars_n8_mod.txt',  # _modwithsky.txt
                scale=0.06, modtype='reg', es=True, mask=fj+'PGC11179_F160W_drz_mask', glx='PGC 11179',
                ryl=1., ylo=3., yhi=8e5)
# sky=skybkgd, DON'T INCLUDE SKY HERE BC IT'S ALREADY INCLUDED IN THE OUTPUT GALFIT MODEL!!!!
# see e.g.: https://github.tamu.edu/joncohn/gas-dynamical-modeling/wiki/Week-of-2020-02-17!!!!
...
print(oop)
#### PGC 11179, adjusted, n9, PA FREE, skyfree '''
''' #### PGC 11179, adjusted, n8, PA FREE
#plot3(gf+'p11179/galfit_out_p11179_reg_linear_hadj_pafree_n8.fits', clipreg=[1300,1500,1370,1570], indivs=False,
#      cliprange=True)
sb_prof_compare(fj+'PGC11179_F160W_drz_sci_adjusted.fits', gf+'p11179/galfit_out_p11179_reg_linear_hadj_pafree_n8.fits',
                gf+'p11179/galfit_out_p11179_reg_linear_hadj_pafree_n8_model.fits',
                gf+'p11179/out_tab_p11179_reg_linear_hadj_pafree_n8_img.txt',
                gf + 'p11179/out_tab_p11179_reg_linear_hadj_pafree_n8_mod.txt', scale=0.06, modtype='reg', es=True,
                mask=None, glx='PGC 11179', ryl=1., ylo=3., yhi=8e5)
print(oop)
...
#plot3(gf+'p11179/galfit_out_p11179_reg_linear_hadj_cb201_n9.fits', clipreg=[1300,1500,1370,1570], indivs=False,
#      cliprange=True)
sb_prof_compare(fj+'PGC11179_F160W_drz_sci_adjusted.fits', gf+'p11179/galfit_out_p11179_reg_linear_hadj_cb201_n9.fits',
                gf+'p11179/galfit_out_p11179_reg_linear_hadj_cb201_n9_model.fits',
                gf+'p11179/out_tab_p11179_reg_linear_hadj_cb201_n9_img.txt',
                gf + 'p11179/out_tab_p11179_reg_linear_hadj_cb201_n9_mod.txt', scale=0.06, modtype='reg', es=True,
                mask=None, glx='PGC 11179', ryl=1., ylo=3., yhi=8e5)  # GUESS X0, Y0 = 1466 1398
print(oop)
#### PGC 11179, adjusted, n9 '''
''' #### PGC 11179, adjusted, n9
